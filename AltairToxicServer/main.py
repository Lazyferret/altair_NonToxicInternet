...
x_train, x_test, y_train, y_test = train_test_split(df_balanced['comment'],df_balanced['toxic'], stratify=df_balanced['toxic'])
...
def start_server():
    ...
    while True:
        try:
            ...
            print(cleared_data)
            #print(type(cleared_data))
            #print(cleared_data[-1])
            #print(type(cleared_data[-1]))
            #print(data[0])
            if data[0][0:4]=='POST':
                print('ok')
                content = []
                for i in cleared_data:
                    content.append("тест")
                client_socket.send(HDRS.encode('utf-8')+str(content).replace("\'","\"").encode('utf-8'))
