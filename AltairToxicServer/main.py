...
def start_server():
    ...
    while True:
        try:
            ...
            data = recvall(client_socket)             #Получение информации от клиента
            #print(type(data))
            print(data)
            data = data.strip().decode(chardet.detect(data)["encoding"])                     #Расшивровка информации от клиента
            print(data)
            ...
            if data[0][0:4] == 'POST':                      #Если запрос - POST
                ...
                #mean = sum(list(predicted_data))/len(list(predicted_data))+0.03
                print(mean)
                for i in range(len(predicted_data)):
                    if predicted_data[i] >= mean and len(cleared_data[i].split()) > 2 and cleared_data[i].strip()!='Выберите режим фильтрации контента' and cleared_data[i].strip().split()[0].lower() not in ['вчера', 'сегодня', 'завтра']:
                        content.append("1")
                    else:
                        content.append("0")
